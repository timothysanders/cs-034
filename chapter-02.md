...
- A **Fibonacci progression* is the sum of the two most recent values (ex. 0, 1, 1, 2, 3, 5, 8, ...). Our class introduces a new attribute `_prev` to store the value prior to the `_current` value. Our constructor expects the first and second values of the progression, with those we can set our `_prev` and `_current` values
